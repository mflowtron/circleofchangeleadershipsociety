...
export default function OrderDetail() {
  ...
  // Sort messages for chat-like display (oldest first)
        ...
        <CardHeader>
          <CardTitle className="text-lg flex items-center gap-2">
            ...
            {unreadCustomerMessages > 0 && (
              <Badge variant="destructive">{unreadCustomerMessages} new</Badge>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
                  ...
                  >
                    ...
                    >
                      <div className="flex items-start gap-2">
                        {!isCustomer && msg.is_important && (
                          <AlertTriangle className="h-4 w-4 text-yellow-600 flex-shrink-0 mt-0.5" />
                        )}
                        <div className="flex-1 space-y-1">
                          <div className={`flex items-center gap-1 text-xs ${isCustomer ? 'text-muted-foreground' : 'text-primary-foreground/70'}`}>
                            {isCustomer ? (
                              <>
                                <User className="h-3 w-3" />
                                <span>Customer{msg.sender_email ? ` (${msg.sender_email})` : ''}</span>
                              </>
                            ) : (
                              <>
                          ...
                          <div className={`flex items-center gap-2 text-xs ${isCustomer ? 'text-muted-foreground' : 'text-primary-foreground/70'}`}>
          ...
          <div className="space-y-3 p-4 border rounded-lg bg-muted/50">
            <Label htmlFor="message">Reply to customer</Label>
            <Textarea
              id="message"
